...
def ler_base_de_dados(iteracao: int, dimensao: int, observacoes: int):
    ...
    caminho_arquivo = (
        f"bases/dimensao_{dimensao}_obs_{observacoes}/base_{iteracao}.json"
    )
...
def retorna_particao_base(
    quantidade_de_agrupamentos: int, quantidade_de_observacoes: int
):
    from mpmath import fdiv
    import numpy as np

    return np.full(
        (quantidade_de_agrupamentos, quantidade_de_observacoes),
        fdiv(1, quantidade_de_agrupamentos),
    )
